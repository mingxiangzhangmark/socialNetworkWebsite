...
export async function batchSetPhoto(photoURL:string){
    ...
    const batch = writeBatch(db);
    try{
        batch.update(profileDocRef, {photoURL: photoURL});
        const querySnapsnap = await getDocs(eventDocQuery);
        querySnapsnap.docs.forEach(eventDoc =>{
        ...
        followingQuerySnap.docs.forEach(fellowDoc =>{
            const fellowDocRef = doc(db, `profiles/${fellowDoc.id}/followers/`, currentUser?.uid as string);
            batch.update(fellowDocRef, {photoURL: photoURL});
        });
        await batch.commit();
    }catch(error:any){
        console.log(error);
